...
Begin VB.UserControl ucScrollbar 
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   106
   ToolboxBitmap   =   "XP_Scrollbar.ctx":0000
End
...
Private Const VERSION_INFO As String = "1.0.0"
...
Private Const TIMERDT_HOT         As Long = 20

Private Const THUMBSIZE_MIN       As Long = 8
Private Const GRIPPERSIZE_MIN     As Long = 12

'-- Private variables:

Private m_bHasTrack               As Boolean
Private m_bRctNoTrack             As Boolean
Private m_uRctNoTrack             As RECT
    ...
    lfHorz = -CLng(m_eOrientation = [oHorizontal])
            ...
            If (UserControl.Enabled) Then
                ...
                If (m_bHasTrack) Then
                    ...
                    End If
                  Else
                    If (m_bRctNoTrack) Then
                        Call pvDrawThemePart(lhDC, SB_THEME, SBP_UPPERTRACKVERT - (2 * lfHorz), HSS_NORMAL, m_uRctNoTrack)
                    End If
                End If
              ...
              Else
                ...
                If (m_bHasTrack) Then
                  ...
                  Else
                    If (m_bRctNoTrack) Then
                        Call pvDrawThemePart(lhDC, SB_THEME, SBP_UPPERTRACKVERT - (2 * lfHorz), HSS_DISABLED, m_uRctNoTrack)
                    End If
                End If
            End If
End Sub
            ...
            With uRct
                If (2 * lButtonSize + THUMBSIZE_MIN > .y2) Then
                    ...
                    If (2 * lButtonSize < .y2) Then
                        ...
                        m_bRctNoTrack = True
                        Call SetRect(m_uRctNoTrack, 0, lButtonSize, .x2, .y2 - lButtonSize)
                      Else
                        Call SetRect(m_uRctTLButton, 0, 0, .x2, .y2 \ 2)
                        Call SetRect(m_uRctBRButton, 0, .y2 \ 2 + (.y2 Mod 2), .x2, .y2)
                        m_bRctNoTrack = CBool(.y2 Mod 2)
                        If (m_bRctNoTrack) Then
                            Call SetRect(m_uRctNoTrack, 0, .y2 \ 2, .x2, .y2 \ 2 + 1)
                        End If
                    End If
                  Else
                    m_bHasTrack = True
            ...
            With uRct
                If (2 * lButtonSize + THUMBSIZE_MIN > .x2) Then
                    ...
                    If (2 * lButtonSize < .x2) Then
                        ...
                        m_bRctNoTrack = True
                        Call SetRect(m_uRctNoTrack, lButtonSize, 0, .x2 - lButtonSize, .y2)
                      Else
                        Call SetRect(m_uRctTLButton, 0, 0, .x2 \ 2, .y2)
                        Call SetRect(m_uRctBRButton, .x2 \ 2 + (.x2 Mod 2), 0, .x2, .y2)
                        m_bRctNoTrack = CBool(.x2 Mod 2)
                        If (m_bRctNoTrack) Then
                            Call SetRect(m_uRctNoTrack, .x2 \ 2, 0, .x2 \ 2 + 1, .y2)
                        End If
                    End If
                  Else
                    m_bHasTrack = True
